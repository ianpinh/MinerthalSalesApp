...
namespace MinerthalSalesApp.ViewModels.Startup
{
    public partial class LoadingPageViewModel : BaseViewModel
    {
        ...
        private async void CheckUserLoginDetails()
        {
            ...
            var usersTask = CarregarUsuariosAsync();
            var sellersTask = CarregarVendedoresAsync();

            Task.WaitAll(usersTask, sellersTask);
        ...
        {
            return App.VendedorRepository.GetTotal();
        ...
        private async Task CarregarVendedoresAsync()
        {
            try
            {
                MessageUsers = "Verificando vendedores...";
                var totalUsers = TotalVendedores();
                if (totalUsers == 0)
                {
                    MessageUsers = "Sincronizando vendedores...";
                    await _servicoDeCarregamentoDasBases.CarregarVendedoresAposLoginAsync();
                    //var users = await App.UserRepository.GetTotalAsync();
                    MessageUsers = "Sincronizando vendedores... Concluído";
                    SalvarLog("Vendedores");
                }
                else
                {
                    MessageUsers = "Verificação concluída...";
